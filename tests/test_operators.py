...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    sigm = sigmoid(a)
    assert 0 <= sigm <= 1, "It is always between 0.0 and 1.0."
    assert (
        eq(1.0 - sigm, sigmoid(neg(a))) == 1
    ), "one minus sigmoid is the same as sigmoid of the negative"
    assert eq(sigmoid(0.0), 0.5) == 1, "It crosses 0 at 0.5"

    sigm_der = sigm * (1.0 - sigm)
    assert sigm_der >= 0, "It is  strictly increasing."
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    assert lt(a, c) == 1 if lt(a, b) == 1 and lt(b, c) == 1 else True
...
def test_distribute(x: float, y: float, z: float) -> None:
    ...
    """
    assert eq(mul(z, (x + y)), add(mul(z, x), mul(z, y))) == 1


@pytest.mark.task0_2
@given(small_floats, small_floats)
def test_other(x: float, y: float) -> None:
    """Only one of x < y and y < y takes place"""
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert eq(sum(addLists(ls1, ls2)), add(sum(ls1), sum(ls2)))
